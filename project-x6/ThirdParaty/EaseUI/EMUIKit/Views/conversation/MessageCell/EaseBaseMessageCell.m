...
#warning 群组里面的头像需要单独设置
        NSUserDefaults *userdefaults = [NSUserDefaults standardUserDefaults];
        NSArray *contactList = [userdefaults objectForKey:X6_Contactlist];
        for (NSDictionary *dic in contactList) {
            if ([[dic valueForKey:@"username"] isEqualToString:model.nickname]) {
                //                model.title = [dic valueForKey:@"name"];
                [self.avatarView sd_setImageWithURL:[NSURL URLWithString:[dic valueForKey:@"avatar"]] placeholderImage:[UIImage imageNamed:@"pho-moren"]];
...
#warning 聊天页面的名字被删
    ...
    NSArray *contactList = [userdefaluts objectForKey:X6_Contactlist];
    for (NSDictionary *dic in contactList) {
        if ([[dic valueForKey:@"username"] isEqualToString:model.nickname]) {
            _nameLabel.text = [dic valueForKey:@"nickname"];
        }
    }
